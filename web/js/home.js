...
$j(function () {
    if (document.documentElement.clientWidth > 465) {
        mainNavConfig = {
            ...
            over: function () {
                $j(this).children('.mainNavFlyout').fadeIn('fast');
                $j(this).children('.parent').addClass('active');
            },
            out: function () {
                $j(this).children('.mainNavFlyout').fadeOut('fast');
                $j(this).children('.parent').removeClass('active');
            }
        }
        $j(".mainNav li").hoverIntent(mainNavConfig);
    }
    else if (document.documentElement.clientWidth < 466) {
        //alert("mobile!");
        $j(".mainNav li a.parent").removeAttr("href");
        $j(".mainNav li a.parent").toggle(function () {
            $j(this).addClass("active");
        }, function () {
            $j(this).removeClass("active");
        });
        $j(".mainNav li a.parent").click(function () {
            $j(this).next("div").slideToggle(200);
        });
    }
    $j("div.mainNavFlyout>div.columnWrapper>div.column>ul>li:last-child").addClass("last");
...
function makeInputSearch(){
	...
	$j(".magniGlass").click(function(){
	    ...
	    //str = str.replace(/[@|^|*|?|<|>|=|#|%|{|}|\[|\]|\:|\;|\/]/g, "");
	    str = str.replace(/[@|'|^|*|?|<|>|=|#|%|{|}|\[|\]|\:|\;|&|\-|"|.|,|\\|`|~|_|!|(|)|$\/]/g, " ");   //    Added By GJ as per ketki's mail new chanracters
	...
	$j("#TextBoxSearch").keypress(function(e){
	    ...
	    str = replaceSpecialCharactersSpace(str);
					window.location = "/Search.aspx?q=" + str;
					return false;
				}
...
/*
 ...
 function replaceSpecialCharactersSpace(str, e) {
